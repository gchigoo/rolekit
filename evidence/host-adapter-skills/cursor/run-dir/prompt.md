...
# Task
- id: RK-ADAPTER-CURSOR
- kind: implementation
- objective: Write src/adapter-cursor.txt via mock executor
...
# Deliverables
- src/adapter-cursor.txt

# Acceptance commands
- `node -e "const fs=require('fs');process.exit(fs.existsSync('src/adapter-cursor.txt')?0:1)"` expect_exit=0
